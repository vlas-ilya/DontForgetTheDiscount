package su.tease.project.feature.cacheback.presentation.preset.cacheback.save.utils
...
import su.tease.project.feature.cacheback.presentation.preset.cacheback.save.action.SaveCacheBackPresetError

class SaveCacheBackPresetForm(
    nameValue: String?,
    infoValue: String?,
    iconPresetValue: CacheBackIconPreset?,
    ...
    val error = mutableStateOf<SaveCacheBackPresetError?>(null)

    val nameError = check(name) {
        require(it.isBlank(), FormFieldError.REQUIRED_BUT_EMPTY)
        require(error.value == SaveCacheBackPresetError.DUPLICATE_ERROR, FormFieldError.DUPLICATE)
    }
    ...
    fun makeResult(cacheBackPresetId: String? = null): CacheBackPreset? =
    ...
    fun ui(block: SaveCacheBackPresetForm.() -> State<FormFieldError?>): State<FormFieldError?> =
