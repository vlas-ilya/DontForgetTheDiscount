...
class StubReducer2 : Reducer<StubState2> {
    ...
    override fun StubState2.onAction(action: PlainAction): StubState2 = when (action) {
    ...
    private fun StubState2.onUpdateIntValue(action: StubAction2.OnUpdateIntValue) =
        copy(intValue = action.intValue)

    private fun StubState2.onUpdateStringValue(action: StubAction2.OnUpdateStringValue) =
